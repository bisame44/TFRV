...
titles = {
    'onsocket': 'Между ядрами',
    'onnode': 'Между сокетами',
    'internode': 'Между узлами',
}
...
def main(*args: str):
    for csvfilename in args:
        ...
        conntype = re.search(r'(?<=-)(\w+)\.csv', csvbasename).group(1)
        plt.plot(x_axis, y_axis, color='blue', linestyle='--', linewidth=1, marker='o', markersize=4, mec='black', mfc='black')
        ...
        plt.title(f'Среднее время передачи пакета\nот одного процесса другому ({titles[conntype]})')
